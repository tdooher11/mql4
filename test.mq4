...
double BBLow,BBMid,BBHigh;
double BBLow_prev,BBMid_prev,BBHigh_prev;
...
int start()
{
       ...
       BBLow=iBands(NULL,0,Period1,2,0,PRICE_CLOSE,MODE_LOW,0);
       BBHigh=iBands(NULL,0,Period1,2,0,PRICE_CLOSE,MODE_HIGH,0);
       BBMid=iBands(NULL,0,Period1,2,0,PRICE_CLOSE,MODE_MAIN,0);
         
       BBLow_prev=iBands(NULL,0,Period1,2,0,PRICE_CLOSE,MODE_LOW,1);
       BBHigh_prev=iBands(NULL,0,Period1,2,0,PRICE_CLOSE,MODE_HIGH,1);
      ...
      if(OrdersTotal()<1)
      {
         ...
         if(prev_bid>BBMid_prev && Bid <= BBMid && trend==1)
         {
      ...
      if(OrdersTotal()>=1)
      {
           //If the price breaks through the mid line close the order
