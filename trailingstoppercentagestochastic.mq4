...
int start()
{
      ...
      for(int cnt=0;cnt<OrdersTotal();cnt++)
      {
         ...
         if(Bid-OrderOpenPrice()>TrailingStop&&OrderType()==OP_BUY) //if current price minus open price is greater than trailing stop value
         {
            if(OrderStopLoss()<Bid-(Bid-OrderOpenPrice())*Percentage) //if current price minus current profit times percentage is greater than current stoploss
            {
